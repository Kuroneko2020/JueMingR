...
namespace Terraria
{
    ...
    internal static class NotesVisualChecks
    {
        internal static void Run(F5FixtureGraphics graphics, string output)
        {
            ...
            NotesHostChecks.WithWorkspace(workspace =>
            {
                ...
                using (var chrome = new F5Renderer())
                {
                    ...
                    var shell = NewShell(); var presentation = new NotesPresentation(workspace);
                    Render(graphics, renderer, chrome, cards, pins, shell, output, "notes-actual-empty", presentation);
                }
            }, Notebook.Empty);
            ...
            NotesHostChecks.WithWorkspace(workspace =>
            {
                ...
                using (var chrome = new F5Renderer())
                {
                    ...
                    var pins = new NotesPins(workspace, renderer, action => workspace.Request(action));
                    var shell = NewShell(); var presentation = new NotesPresentation(workspace);
                    Render(graphics, renderer, chrome, cards, pins, shell, output, "notes-actual-browse", presentation);
                    workspace.RequestDelete(other.Id);
                    Render(graphics, renderer, chrome, cards, pins, shell, output, "notes-actual-delete-confirmation", presentation);
                    ...
                    Render(graphics, renderer, chrome, cards, pins, shell, output, "notes-actual-selection", presentation);
                    workspace.CancelEdit(); workspace.Feature.AdjustReading(note.Id, true, 7); workspace.Feature.AdjustReading(note.Id, false, 6);
                    Render(graphics, renderer, chrome, cards, pins, shell, output, "notes-actual-large-reading", presentation);
                    workspace.Feature.AdjustReading(note.Id, true, -8); workspace.Feature.AdjustReading(note.Id, false, -10);
                    Render(graphics, renderer, chrome, cards, pins, shell, output, "notes-actual-small-reading", presentation);
                    Render(graphics, renderer, chrome, cards, pins, shell, output, "notes-actual-footer-last-line", presentation, footer: true);
                    Render(graphics, renderer, chrome, cards, pins, shell, output, "notes-actual-scale150", presentation, 1920, 1080, 1.5f);
                    Render(graphics, renderer, chrome, cards, pins, shell, output, "notes-actual-narrow-scale150", presentation, 1000, 900, 1.5f);
                }
            }, new Notebook(new[] { note, other }));
            foreach (bool unknown in new[] { false, true }) NotesCopyChecks.WithSaveFailure(unknown, (workspace, presentation) =>
            {
                using (var renderer = new NotesRenderer())
                using (var chrome = new F5Renderer())
                {
                    ...
                    var cards = new NotesCards(workspace, input, renderer, action => workspace.Request(action));
                    var pins = new NotesPins(workspace, renderer, action => workspace.Request(action));
                    Render(graphics, renderer, chrome, cards, pins, NewShell(), output,
                        unknown ? "notes-actual-save-unconfirmed" : "notes-actual-save-failed", presentation);
                }
            });
        ...
        private static void Render(F5FixtureGraphics graphics, NotesRenderer renderer, F5Renderer chrome, NotesCards cards, NotesPins pins,
            F5Interaction shell, string output, string name, NotesPresentation presentation, int width = 1440, int height = 960, float scale = 1, bool footer = false)
        {
            Color statusColor; string status = NotesCopyChecks.Feedback(presentation, out statusColor);
            ...
            for (int i = 0; i < 30; i++) { renderer.BeginLayoutFrame(); cards.Prepare(shell, status, statusColor); pins.Prepare(width, height); }
            if (pins.Pins.Count != 0)
            {
                NotesPin pin = pins.Pins[0];
                F5Rect region = pin.Body;
