...
'@, (New-Object Text.UTF8Encoding($false)))
        ...
        foreach ($dataProfile in @('phase0v-settings', 'phase0w-notes', 'item-automation', 'unified-hotkeys', 'entity-labels', 'world-targets', 'world-object-text', 'information-summary', 'direction-equipment', 'death-history', 'map-markers-exploration', 'footprints', 'item-browser', 'favorite-quick-items', 'coin-deposit', 'about-help-feedback')) {
        ...
        foreach ($dataTiming in @('before-install', 'after-install')) {
            $dataTarget = New-Phase0STargetDirectory -Root $root -TerrariaIdentityInput $terrariaIdentityInput -Name ($dataProfile + '-data-' + $dataTiming)
            ...
            foreach ($step in @('before-install', 'install', 'after-install')) {
                if ($step -eq 'install') {
                    Assert-Phase0SCompactJsonResult -Result (Invoke-Phase0SPackageScript -PackageRoot $dataPackageRoot -ScriptName 'Install-Phase0S.ps1' -TerrariaDirectory $dataTarget) -Operation 'install' -ExpectedExitCode 0 -ExpectedCode 'INSTALL_COMPLETE' -ExpectedStatus 'success' -TargetDirectory $dataTarget
                    Assert-Phase0SReceiptMatchesPackageManifest -PackageRoot $dataPackageRoot -TargetDirectory $dataTarget
                ...
                elseif ($step -eq $dataTiming) {
                    ...
                    [System.IO.Directory]::CreateDirectory((Join-Path $dataRoot 'behavior\onboarding')) | Out-Null
                    [System.IO.File]::WriteAllText((Join-Path $dataRoot 'behavior\onboarding\opaque.json'), '{"version":999,"keep":"onboarding-user-data"}', (New-Object System.Text.UTF8Encoding($false)))
            ...
            Write-Phase0SEvidence -TargetDirectory $dataTarget -PackageId ([string] $dataManifest.packageId) -Kind 'complete'
            Assert-Phase0SCompactJsonResult -Result (Invoke-Phase0SPackageScript -PackageRoot $dataPackageRoot -ScriptName 'Restore-Phase0S.ps1' -TerrariaDirectory $dataTarget) -Operation 'restore' -ExpectedExitCode 0 -ExpectedCode 'RESTORE_COMPLETE' -ExpectedStatus 'success' -TargetDirectory $dataTarget
