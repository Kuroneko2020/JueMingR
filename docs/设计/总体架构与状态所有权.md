...
推荐方案吸收纯纵向切片的局部内聚，但不允许每个功能重复 Host、线程、identity、操作安全和机械存储；吸收中央 Runtime 的单一 composition 和生命周期泵，但拒绝无类型 Event Bus、Service Locator、全局快照和插件内核。
...
- 宿主无关平台；
- Feature 集合；
- Terraria Host 与 composition root；
...
| Composition / Runtime Lifecycle | 在 Host 晚期激活阶段验证核心宿主条件、构造对象图，并拥有 Runtime、Session 和 Feature activation 生命周期 | 宿主无关平台合同；composition root 可认识具体 adapters/Features | 具体 Feature 业务规则、Service Locator、任意全局解析 | Runtime、Session、Feature activation 状态 | Feature 产品清单、内部状态、设置语义、游戏事实 | 唯一 Bootstrap handoff 后至 Runtime stop；发布明确生命周期回调 | 逆序释放已取得资源；核心失败整体不激活，单 Feature 失败局部停用 |
...
| Terraria Host Adapter | 唯一直接适配 Terraria、ReLogic、XNA、Harmony；在固定 handoff 后承担 Host 晚期激活，并实现观察、最小操作、Hook、渲染和输入宿主端口 | 宿主无关平台端口；composition 区域可引用具体装配对象 | Feature 业务规则、持久化语义、Diagnostics 决策 | Hook 句柄、宿主连接、线程身份和 probe evidence | Terraria 原始事实、Feature 可用性或运行状态 | Host 晚期激活至 Runtime stop；提供 typed host ports | 释放已取得句柄；报告 capability 或 operation 失败 |
...
| Operation Coordination | 唯一接收游戏 mutation intent，管理 admission、队列、执行和终态 | Host operation ports、Session/capability/activation view | Feature 目标选择、产品规则、任意字符串业务分派、所有 Feature 状态 | 请求、冲突、队列、执行与结果状态机 | Feature 业务状态、Terraria 权威事实 | Session；typed request/result | 返回失败、取消、超时、部分完成或未确认；不假成功 |
...
| Presentation | 渲染 Feature/Settings 提供的只读 ViewModel，维护布局与交互瞬时状态，并把用户操作转发为明确命令 | Feature/Settings presentation contracts；Host renderer | 重新解释业务状态、直接写 Terraria、直接写文件、以控件状态拥有业务事实 | 窗口、滚动、layout、hit-test 等瞬时展示状态 | 语义 ViewModel、Feature 业务状态、持久事实、游戏事实 | UI/Session；只读 model + command | 显示 unavailable/error；不把错误伪装为空态 |
...
flowchart TB
    ...
    Host[Terraria Host + Composition Root]
    Platform[Host-neutral Platform]
    Features[Feature Collection]
    Infra[Local Infrastructure]
    Setup[Setup / Recovery]
    Game[Terraria.exe / ReLogic / XNA / Harmony]

    Bootstrap -. 单次精确晚绑定交接 .-> Host
    Host --> Platform
    Host --> Features
    Host --> Infra
    Features --> Platform
...
只有 `Terraria Host + Composition Root` 可以引用游戏二进制与 Harmony。Composition root 是唯一可以同时认识具体 Feature 和具体 adapter 的位置；其余 Host adapter 代码只能依赖宿主无关端口与 presentation contracts。
...
| Host-neutral Platform | 自身内部的稳定合同 | 生命周期、状态与端口 |
...
| Composition root | Platform、Features、Local Infrastructure、Host adapters | 显式构造对象图 |
...
| 当前玩家/世界/服务器 Session 身份 | Runtime Lifecycle 的 Session coordinator | Session coordinator | Capability、Observation、Operation、Feature、Persistence、UI | 进入世界至退出/切换 | 身份本身不作为独立业务事实持久化；可作为 scoped key | 离开、切角色/世界/服务器、断线；换 generation | NoSession/IdentityUnavailable | 单一 Session scope、cancellation、generation tests |
| Runtime 生命周期 | Runtime Lifecycle | Runtime coordinator | 全部模块的只读 lifecycle view | 进程内 handoff 至 stop | 否 | stop 或 core failure | Starting/Active/Stopping/Stopped/Failed | state machine、composition tests |
| 核心 capability | Capability / Compatibility | Core capability evaluator | Runtime、Feature availability evaluator、Presentation projector；Diagnostics 只收主动 publication | 进程或 host identity 代次 | 否；验证证据另存，不成为运行事实 | host identity/版本/assembly 变化、显式重探测 | Unknown/Available/Unavailable(reason) | typed capability keys、runtime gate |
| 可选 Host capability | Capability / Compatibility | Shared capability evaluator | 声明依赖的 Feature、Runtime、Presentation projector；Diagnostics 只收主动 publication | 进程或 Session | 否 | probe source、Host identity 或 Session 变化 | Unknown/Available/Unavailable(reason) | typed capability keys、probe/gate tests |
| Feature 产品清单与稳定 identity | Feature Collection 的显式 composition manifest | manifest 定义；composition 只验证 | Composition、Settings、Presentation projector、Diagnostics | 一个产品构建身份 | 否 | 新构建替代 | Duplicate/Invalid 时 composition fail closed | 编译期声明、唯一性架构测试 |
| Feature 可用性与降级结论 | 对应具体 Feature | Feature availability state machine | Runtime activation、对应 Presentation projector；Diagnostics 只收主动 publication | 进程或 Session | 否 | 依赖 capability、Session 或业务前置证据变化 | Unknown/Available/Unavailable/Degraded(reason) | requirement declaration、Feature gate tests |
...
| Feature 实际激活状态 | Runtime Lifecycle | Feature activation coordinator | Feature、对应 Presentation projector；Diagnostics 只收主动 publication | Runtime/Session | 否 | 期望开关、可用性、Session、失败、stop | Disabled/Unavailable/Starting/Active/Stopping/Failed | lifecycle interface、state tests |
...
| Operation 请求、排队、执行与结果 | Operation Coordination | Operation state machine | 请求 Feature、Runtime；Diagnostics 只收有限主动 publication | request + Session generation；终态有界保留 | 不作为业务事实持久化；可进入有限诊断 | terminal TTL、Feature disable、Session exit、Runtime stop | Rejected/Queued/Executing/Succeeded/PartiallySucceeded/Failed/Cancelled/TimedOut/Unconfirmed | sealed typed request/result、internal executors、contract tests |
...
| UI 布局与交互瞬时状态 | Presentation | 对应 UI state owner | Presentation Host | UI interaction | 否 | 隐藏、交互完成、viewport/scale/坐标域变化 | Cancelled/InvalidLayout | internal state、UI tests |
...
| Presentation layout/render cache | Presentation Host Adapter | 对应 renderer | renderer/input hit-test | frame、viewport、scale、content revision | 否 | viewport/坐标域/content 变化、UI 隐藏 | Rebuild/Unavailable | coordinate-domain types、UI tests/实机 |
...
## 13. 进程、Bootstrap 与 Runtime 生命周期

总体启动链只跨越一次晚绑定所有权交接：Early Bootstrap 在明确宿主条件满足后，调用固定的 Terraria Host handoff；Host 内的“晚期激活”是该 handoff 的接收阶段，负责核心 probe、单一 Harmony 上下文、宿主回调和 composition，不是第二个 Bootstrap 或第二次动态交接。
...
| Host 晚期激活 | Terraria Host + Composition | 核验宿主、安装最小句柄并进入 composition；失败时逆序释放，不进入 Runtime Active |
| Runtime composition 与核心能力检查 | Runtime Lifecycle + Capability | 构造对象图并得到明确 core capability；失败进入 `FailedStopped(reason)` |
| 主菜单 | Runtime Lifecycle | `ActiveWithoutSession`；允许进程级设置和 UI，不运行 Session Feature |
| 进入角色与世界 | Session coordinator | 建立 generation、identity、Session capability 与 scoped state |
| Tick/frame | Terraria Host + Runtime pump | 游戏线程推进到期观察、Feature 和 operation；Draw 只消费投影 |
| 世界退出、断线或角色/世界切换 | Session coordinator | 进入 Leaving，取消旧 generation、清理并回到 `ActiveWithoutSession` |
| 单项 Feature 失败 | 对应 Feature + activation coordinator | 该 Feature `Unavailable/Failed(reason)` 并局部清理；Runtime 继续 |
| Runtime stop | Runtime Lifecycle | 拒绝新工作，停止 Session/Feature，逆序释放 Host 资源后 `Stopped` |
...
任一 Runtime 启动阶段可以进入 `FailedStopped(reason)`，但不能进入“部分 Active”。Composition 按取得顺序记录资源句柄；核心失败时逆序释放已经安装的 Hook、取消工作并停止 adapter。Feature 启动失败只关闭该 Feature，不改变 Runtime 核心状态。
...
一个 Session 表示当前本地玩家、世界和多人端点的组合上下文。主菜单期间 Runtime 可以 Active，但没有 Session。
...
Host Adapter 在开发/测试构建中提供线程断言；Operation Coordinator 是后台请求回到游戏线程的唯一入口。
...
- 解释用户行为合同；
- 选择目标和请求时机；
- 形成 typed intent/request；
- 解释 `PartiallySucceeded`/`Unconfirmed` 等结果对该功能的意义；
- 更新自己的领域状态和用户可见反馈。
...
- 当前 Session generation、Feature activation 和 capability 复核；
- game-thread switching；
- admission、冲突、串行化和有限容量；
- 幂等 key、重复、过期、取消、超时和世界切换；
- 通用安全前置条件；
- 调用最小 Host executor；
- 结果分类和有限诊断。

它不得包含具体 Feature 的目标选择、产品规则、UI、持久化或字符串式任意动作分派。Host executor 只能实现经过登记的 typed host operation，不接受自由字符串或反射式业务命令。
...
```text
Rejected | Queued → Executing → Succeeded
                              → PartiallySucceeded
                              → Failed
                              → Cancelled
                              → TimedOut
...
sequenceDiagram
    ...
    A->>A: session/capability/conflict/timeout 复核
    A->>H: 执行最小 host operation
    H->>T: 原版游戏调用/多人请求
    T-->>H: 本地结果或权威回执
...
Feature identity 必须进入 Feature Collection 的显式产品清单；composition 只能枚举并验证该清单，不得在 Runtime 中另建硬编码目录。重复或无效 identity 必须在 composition 时 fail closed，不能静默覆盖。
...
→ Operation Coordinator 复核并串行
→ Host executor 调用原版操作
→ Succeeded/PartiallySucceeded/Failed/Cancelled/TimedOut/Unconfirmed
→ Feature 状态与用户反馈
```

Feature 负责“何时、选什么、产品上算什么结果”；Operation Coordinator 不知道自动增益或自动堆叠规则。
...
| Feature 不直接写游戏 | internal Host executors、typed operation port | contract/integration、Terraria 实机 |
...
2. 落实第 7 节的有向引用图；
3. 只允许 Terraria Host 项目接收合法游戏引用和 Harmony；
4. 建立一个最小架构测试项目，验证禁止引用和 Legacy 零依赖；
5. 只放置能证明边界的空 composition/handoff seam，不创建业务 Feature；
6. 锁定工具链、TFM、PlatformTarget、合法引用输入和构建身份；
7. 保持 Setup/Recovery 与游戏内项目独立，但不在该阶段实现安装器。
...
- composition/DI 的具体实现；
- UI 框架、控件、视觉、布局、DPI 和坐标转换实现；
- 各 Feature schema、文件格式、路径和 Legacy 数据导入；
- observation cadence、cache 容量、operation conflict classes 和队列容量；
- 测试框架、分析器、CI 和性能预算；
- 首个代表性纵向切片。
