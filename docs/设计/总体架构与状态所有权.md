...
- 状态：有效
- 生效边界：本设计随包含本文件和 ADR-0010 的提交合并进入 `main` 后，成为仓库当前有效总体架构。
- 授权边界：本设计规定总体架构、模块依赖和状态所有权边界；任何项目骨架、生产代码、测试、依赖、构建、CI、安装器或具体功能仍需独立 Issue 和项目所有者授权。
...
本设计描述 JueMingR 当前总体系统应怎样组织，目标是指导下一阶段建立最小项目骨架，并让后续功能在不复制 JueMingZ 内部结构的前提下逐项进入。
...
- JueMingR 是原版 Terraria 客户端辅助工具；独立重写描述与 JueMingZ 的工程关系，旧版功能不构成产品上限。定位说明见[根 README](../../README.md)；本条背景澄清已随 [ADR-0015（Accepted）](../决策/ADR-0015-产品定位与能力演进边界.md) 获所有者接受，按其合并生效边界执行，其余有效架构、依赖方向和状态所有权不变。
...
延期不表示这些事项不重要；只是它们分别需要项目骨架、目标 Terraria ABI、具体 Feature 合同或运行测量，不能由当前静态架构设计猜定。
...
项目所有者已经接受本设计承载的总体架构语义；本设计只有随包含本文件和 ADR-0010 的提交合并进入 `main` 后，才成为仓库当前有效总体架构。本次接受不授权项目骨架或生产代码，也不表示项目引用、编译、加载、Terraria 实机、多人或性能已经验证。
