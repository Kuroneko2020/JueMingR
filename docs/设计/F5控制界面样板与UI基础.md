...
本批关于页与群号复制操作按[关于帮助与反馈设计](关于帮助与反馈实现设计.md)接入现有第六分类，共享主窗、滚动、裁切及按下/释放代次。复制结果在原按钮短暂显示约3秒后恢复群号；首次提醒复用原版 PopupText 浮字队列，不开启独立窗口、不恢复普通功能状态行。接受边界查 [#95](https://github.com/Kuroneko2020/JueMingR/issues/95)。
...
历史样板段落中的“无业务键盘/三条显名行”只描述当时范围；原四项已经由 #51 公共系统接入。#55 在信息页现有前三行增加真实幂等命令、NPC 名字/类型模式、独立配置入口及公共键盘入口，保留主窗口尺寸、导航、群系行和主滚动参数。

`StylePopup` 只拥有一个目标和未提交编辑，使用原 F5 字体、皮肤、按钮与度量；HSL 真实释放、六位 HEX 完成、字号按钮分别进入功能设置。内容决定窗高，错误行出现不丢 HEX 草稿；viewport/font/skin/入口几何变化取消失效手势。文字租约复用 HostInputState 和原版提交字符队列，IME 控制动作优先但不丢确认字符；Esc 先结束编辑，再关闭副窗。样式和绑定窗互斥，遮挡区与完整手势尾部阻止下层操作。所有者已正式接受c1候选的三项显示、配置副窗及快捷键接入，随PR #56合并进入主线。交付环境在设备创建前不可用、代表预览未查看的记录保留，不由体验接受补造图形自动检查PASS；具体边界见[显名合同](../功能介绍/敌怪动物与NPC显名功能行为合同.md#85-实施与接受的边界)和[实现设计](实体显名与样式配置实现设计.md)。

## 普通功能行说明

所有者已实测可用并正式接受候选 `968ae079e2207a7954bb7e271e4a7b8ba371ead9` 的名称悬停说明、按钮提示整理及信息页和物品页共用提示机制。本接入约定自 [PR #74](https://github.com/Kuroneko2020/JueMingR/pull/74) 合并进入 `main` 起正式生效；既有有效设计继续适用。接受仅覆盖实际体验与对应交付，不补造尚未执行的图形检查、全部字体/材质、Windows/IME、多人或 FPS 验证；证据与接受范围见 [#73](https://github.com/Kuroneko2020/JueMingR/issues/73)。

普通功能行的用途概括与必要使用条件，由功能名称的悬停说明承载；含义明确的文字按钮不重复解释。图标、特殊操作和必要状态保留对应提示；复杂界面按实际需要提供上下文帮助，关键风险和错误反馈不得仅依赖悬停。

功能模块在已有 Controls 或 Items 呈现入口提供一份静态简介，复用模块稳定身份，经 `F5RowLayout.Row` 的可选说明元数据传给名称文本；普通行新增时须核对摘要和必要条件，不按枚举为未来占位强制补齐。共享布局只管理实际文字范围、少量留白与坐标，名称不成为命令。信息与物品两条真实呈现路径统一复用提示排版，模式特有解释、键盘识别、不可用原因以及复杂界面原提示仍由各自入口负责。

新增或实质修改功能行开关／主模式时，默认同时落实行尾公共绑键入口和快捷键真实生效短提示；其它操作结果先核对适用通道。实施与审查必须读取[后续功能的默认接入](../功能介绍/本地短提示与快捷键反馈行为合同.md#后续功能的默认接入)和[技术接入](本地短提示与快捷键反馈实现设计.md#后续消费者接入)，不以本批 37 项为未来白名单。此为 #101 本批已接受合同补充，合并生效边界同该合同；普通 F5 点击静默、不预留状态行以及既有特殊交互约定保持。

普通功能名称提示统一使用公共机制；页面提供内容与有效名称区域，保留自身业务布局和交互。新增页面不得复制提示实现。`ItemsLayout.Project` 保留行说明和名称区域，`ItemsPresentation.Hint` 只做薄适配；F5 壳层唯一 `F5Renderer.DrawHints` 经共同 `PrepareHints` / `F5HintLayout` 管理当前排版、缓存、定位、显示和清除。内容绘制和名称命中复用 `F5ControlRenderer.ContentClip` 的最终像素裁切；物品卡片原上下文提示、选择器及模态副窗仍保留各自必要阶段与交互。

提示只在当前可见名称范围命中，随原窗口/滚动/缩放投影，模态副窗、拖动、编辑或失焦时让路；不持有输入或改变页面、业务及保存状态。相同内容、字体、字号与可用宽度复用当前一条排版，位置变化只投影，不将动态错误无限存入固定文案缓存。代表回归由 `tests/Phase0U/F5NameHintChecks.cs` 与 `tests/Items/ItemUiChecks.cs` 承担，沿既有布局/物品及正常工作量入口执行。

## 玩家可见文案

本节长期原则已获所有者正式接受，随 [PR #78](https://github.com/Kuroneko2020/JueMingR/pull/78) 合并进入 `main` 后生效；[Issue #77](https://github.com/Kuroneko2020/JueMingR/issues/77#issuecomment-5676082124)记录接受依据。既有开发与独立审查入口继续通过短路由落实，不改变此前有效规则。

本约定适用于 F5、HUD、弹窗、操作反馈及加载、保存和错误状态。玩家文案以用途、操作与当前结果为中心，使用自然游戏语言。保留必要条件、模式差别、操作顺序和有用反馈；省略内部实现、明显表现、穷举排除和无必要的叮嘱。简短不以误导为代价，完整开发合同不应成为玩家阅读负担。

功能名称承载简介，图标和特殊操作提供必要帮助，状态区说明当前结果。常规成功不夹带教学；错误不伪装成成功。“正在加载”“没有目标”“保存失败”“无法确认是否保存成功”各有含义，不为精简合并。失败只在确有支持时给出可执行的下一步，不统一追加重试、退出游戏或检查文件的要求。玩家自写内容和原版、资源包文本保持其原有来源。

按 G06 #93 追加的产品呈现裁决，普通功能行不自动展示内部状态、正常等待或短暂保存消息，也不为这些消息预留高度；“状态区说明结果”不表示每个普通功能都自带状态区。真实风险／错误使用合适的一次本地提醒及现有名称悬停回看，不因静默而删除业务保护；真实子界面、必要空结果和结果列表按其用途保留。相邻完整功能块直接交接公共行已经包含的尾部间距，绘制、命中、滚动与悬停共用最终布局；纯说明变化不要求重建几何。本段是产品呈现约定，不增加网页讨论或本地开发流程。

实现者应沿真实显示入口核对完整拼接结果，在功能原有静态文案或有限格式化处修改，复用既有提示、布局与缓存。内部原因码继续服务原有判断和诊断，在自然呈现边界转述；不在 Draw 中过滤、替换或扫描文字，不建立平行文案平台。没有所有者逐字提供的文案时，实现者仍负责定稿。

本轮例子说明取舍，不规定固定句式或禁词：

- “设置快捷键”改为“双击设置功能开关快捷键”，虽更长，却说明了真实手势与用途；一次动作另写“双击设置调整信息窗位置的快捷键”，不能称作开关。
- “显示碎岩龟”的简介应指向睡眠中的碎岩龟；人工样本复制成“魔力水晶”是笔误，不据此改变检测对象。
- 幸运值和渔夫任务保留“在此世界解救过，或当前存在”的条件；树妖仍要求当前存在。装备提示概括 Boss 战及指定事件、固定的部分非战斗用品，不承诺判断所有配装或所有事件。
- 已明确删除游商测试下方“生成真实旅商，会影响当前世界；建议在测试世界使用”和成功后的“请手动开启旅商方向”。不以同义句转移到其他入口；已确认到访只需“旅商已到访。”，未确认结果不能使用成功句。

独立审查须阅读实际最终文本及其上下文，核对关键含义、动态参数、显示分支和未填样本以外的可达入口。自动测试负责适合机器的状态对应、模板参数、必要操作、未知与失败的区别、布局缓存及明确删除内容的回归；自然表达由有限的阅读审查负责，不用字数、禁词或文案评分代替判断。原则正文只维护于此，开发及审查入口通过短路由引用。
...
本批本地短提示：[#101](https://github.com/Kuroneko2020/JueMingR/issues/101) 的已接受实现只为合法热键的真实开关变化新增有限本地回执，不改变本页普通 F5 操作与加载／保存的静默约定，不增加行高。短操作结果、持续错误、输入校验和外部宣告各自通道及限定体验接受见[本地短提示合同](../功能介绍/本地短提示与快捷键反馈行为合同.md)，随 [PR #102](https://github.com/Kuroneko2020/JueMingR/pull/102) 合并进入 `main` 生效。
