...
已交付范围是 F5 基础壳层与群系显示真实控制；十二个分类入口不代表十二类 Legacy 功能全部迁移。尚未接入的业务保持不可执行，持久化、完整功能迁移、多人验证与正式发行均未完成。整体体验接受仅覆盖所有者实际测试场景，与自动、实机分项、多人和性能证据分别记录，不推导全部材质包、分辨率、机器或 FPS/慢帧合格。
