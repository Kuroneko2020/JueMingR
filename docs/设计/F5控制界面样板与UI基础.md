...
`StylePopup` 只拥有一个目标和未提交编辑，使用原 F5 字体、皮肤、按钮与度量；HSL 真实释放、六位 HEX 完成、字号按钮分别进入功能设置。内容决定窗高，错误行出现不丢 HEX 草稿；viewport/font/skin/入口几何变化取消失效手势。文字租约复用 HostInputState 和原版提交字符队列，IME 控制动作优先但不丢确认字符；Esc 先结束编辑，再关闭副窗。样式和绑定窗互斥，遮挡区与完整手势尾部阻止下层操作。图形环境已在设备创建前确认不可用，代表预览未查看；本节不构成外观或实机接受。实际状态见[实现设计](实体显名与样式配置实现设计.md)和 #55。
