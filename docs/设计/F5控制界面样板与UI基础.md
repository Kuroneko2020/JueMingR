...
基础验收范围是 F5 壳层与群系显示真实控制；十二个分类入口不代表十二类 Legacy 功能全部迁移。尚未接入的业务保持不可执行。该基础验收不包含配置持久化、完整功能迁移、多人验证或正式发行；整体体验接受仅覆盖所有者实际测试场景，与自动、实机分项、多人和性能证据分别记录，不推导全部材质包、分辨率、机器或 FPS/慢帧合格。窗口位置与用户期望开关的持久化扩展按 [Issue #34](https://github.com/Kuroneko2020/JueMingR/issues/34) 和[配置持久化设计](配置持久化与用户数据布局.md)的独立实施、接受及生效边界办理。
