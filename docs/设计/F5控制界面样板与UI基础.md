...
状态：已获项目所有者基础体验接受；随本次 PR #31 合并进入 `main` 后成为当前有效的 F5 UI 基础设计。接受与交付证据、第四 Hook 授权见 [Issue #30](https://github.com/Kuroneko2020/JueMingR/issues/30)，阶段路由见 [Issue #7](https://github.com/Kuroneko2020/JueMingR/issues/7)。本文件不改变既有总体架构、加载链或已接受 ADR。

项目所有者确认当前 UI 测试已完成，在本人已测试的使用场景中未发现使用问题，接受当前基础作为后续开发起点。该接受不等于外观永久定稿；未来颜色、间距、图标和布局改进不追溯性阻塞本次基础交付。文字不可读、输入错位、点穿、异常裁切等仍是实际缺陷，不能一概归为美化。

基础验收范围是 F5 壳层与群系显示真实控制；十二个分类入口不代表十二类 Legacy 功能全部迁移。尚未接入的业务保持不可执行。该基础验收不包含配置持久化、完整功能迁移、多人验证或正式发行；整体体验接受仅覆盖所有者实际测试场景，与自动、实机分项、多人和性能证据分别记录，不推导全部材质包、分辨率、机器或 FPS/慢帧合格。窗口位置与用户期望开关的持久化扩展已在 [Issue #34](https://github.com/Kuroneko2020/JueMingR/issues/34) 中独立获项目所有者接受，随 [PR #35](https://github.com/Kuroneko2020/JueMingR/pull/35) 合并进入 `main` 后成为当前主线能力；具体合同与接受边界见[配置持久化设计](配置持久化与用户数据布局.md)。
...
Settings 保存用户期望，Feature 保持上述实际状态与故障边界。恢复关闭值不触发故障，读取期望开启不绕过故障锁，故障不保存为用户关闭。对应偏好读取完成前拒绝操作；正常读取、修改和保存静默，真正异常按文档与原因去重后提醒，具体交付边界见[配置持久化设计](配置持久化与用户数据布局.md)。写保护不因会话内调整而解除。UI 不直接操作 JSON，也不为未接入控件预建设置。
...
`F5Interaction.OwnsPointer` 保存唯一窗口/捕获归属事实；Host 再统一核验当前能否实际呈现 F5。全屏地图、隐藏 UI、Fancy UI、游戏选项、相机、失焦和非键鼠模式均关闭 F5；当前地图/相机切换请求也先取消控件与捕获，继续处理已归属按钮的真实释放。原基础范围为单人；#42 笔记候选将共享壳层窄开放给 netMode 0/1 的本地客户端，群系控制与运行仍有各自 netMode 0 门，不据此宣称其它入口多人安全。NPC prefix 不存第二份遮挡标志，不做反射、日志、文字度量、I/O 或实体扫描。

笔记候选沿用一个 F5 layer，依次以屏幕矩阵绘制便签、以冻结 UI 矩阵绘制窗口和卡片。独立文本状态、共享布局预算、正文与页滚动、主动离开保存门及强制隐藏留稿按[笔记设计](笔记功能实现设计.md)；F5 关闭后便签仍借用可用原版资源，不 dispose 共享字体/纹理。文本租约在原输入目标增加最小 prefix，与既有 postfix 共用同一 Harmony owner；原首四个宿主目标集合不扩大。真实 IME、像素及多人证据以该任务实际验证为准。
