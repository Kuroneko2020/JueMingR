...
新增或实质修改功能行开关／主模式时，默认同时落实行尾公共绑键入口和快捷键真实生效短提示；其它操作结果先核对适用通道。实施与审查必须读取[后续功能的默认接入](../功能介绍/本地短提示与快捷键反馈行为合同.md#后续功能的默认接入)和[技术接入](本地短提示与快捷键反馈实现设计.md#后续消费者接入)，不以本批 37 项为未来白名单。此为 #101 本批合同补充，接受与合并边界同该合同；普通 F5 点击静默、不预留状态行以及既有特殊交互约定保持。
