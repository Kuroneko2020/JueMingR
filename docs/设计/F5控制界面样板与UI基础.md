...
信息页仅在“显示龙蛋”与“调整信息窗位置”之间加入一条内容分隔，左右内缩 12，计入内容高度并跟随滚动/裁切，不可点击。它与固定标题分隔分别属于内容和窗口。功能行尾部键位入口为较小的自绘键盘几何，在原有有界槽位中居中，没有按钮底色、额外边框或文字“键”；仅保留图案自身轮廓，仍无绑定/录入/弹窗/持久化业务。十二个导航图标的资产与管线保持既有实现。
