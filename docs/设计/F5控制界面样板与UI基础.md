...
Settings 保存用户期望，Feature 保持上述实际状态与故障边界。恢复关闭值不触发故障，读取期望开启不绕过故障锁，故障不保存为用户关闭。对应偏好读取完成前拒绝操作，完成后内存应用与磁盘保存分别通过有限中文提示反馈；写保护不因会话内调整而解除。UI 不直接操作 JSON，也不为未接入控件预建设置。
