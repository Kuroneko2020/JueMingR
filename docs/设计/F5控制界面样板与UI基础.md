...
本批关于页、页内帮助与复制操作按[关于帮助与反馈设计](关于帮助与反馈实现设计.md)接入现有第六分类，共享主窗、滚动、裁切及按下/释放代次。复制结果在原按钮内保留，失败详情用公共提示；首次入门提醒为独立非模态呈现，不恢复普通功能状态行。接受边界查 [#95](https://github.com/Kuroneko2020/JueMingR/issues/95)。
