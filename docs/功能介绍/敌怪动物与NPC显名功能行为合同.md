# 敌怪、动物与 NPC 显名：Legacy 行为与 R 已接受实现

**性质：Legacy 静态行为提取与 R 已接受实现。所有者于2026-09-11接受第8节用户结果与明确差异，并于2026-09-12正式接受候选 `c1b718cc3df1d93b544a1ea14fde5e928a070ebe` 的三项显名、配置副窗和快捷键接入，随 PR #56 合并进入主线。第2–7节旧代码、历史记录与风险没有整体升级为新版施工规定或运行验证。** 接受与验证边界见[第8.5节](#85-实施与接受的边界)，需求确认见[任务 #53](https://github.com/Kuroneko2020/JueMingR/issues/53)，实际收口见[任务 #55](https://github.com/Kuroneko2020/JueMingR/issues/55)和[PR #56](https://github.com/Kuroneko2020/JueMingR/pull/56)，当前阶段只查[总跟踪 #7](https://github.com/Kuroneko2020/JueMingR/issues/7)。
...
提取时 R 可复用 F5 行控件、字体度量、输入仲裁、偏好保存、统一键鼠快捷键及 Session 生命周期。提取基线的三条显名行只是外形；没有真实后端或动作注册。实体观察和世界标签绘制需最小补齐，不建设全游戏实体平台。已确认产品目标集中在[第 8 节](#8-已确认的新版首轮目标)，技术实现选择由后续实施者依据当前版本证据设计。
...
- 三项名称字号默认0.90（2026-09-12所有者微调），范围0.50–1.80，按钮步长0.10，夹限且避免累计漂移；血量字号为名称−0.13，保留两位并夹0.50–1.80，默认0.77，名称可调范围内血量上限1.67。单位为缩放倍数，不是pt；不要求复制旧行高/测量公式。新默认适用于首次缺失偏好及恢复默认，不覆盖已有保存值。
...
所有者已实测并于2026-09-12正式接受候选 `c1b718cc3df1d93b544a1ea14fde5e928a070ebe` 的三项显名、配置副窗与公共快捷键接入，授权必要文档最终化后正常合并。F5信息页前三行提供显示/模式、独立颜色与字号配置及原行末双击快捷键入口；三项首次关闭、新键未绑定，NPC再开恢复最后明确模式。名称默认0.90、敌怪血量默认0.77；已有保存值不覆盖，恢复默认同时恢复当前项颜色和字号。HSL拖动只预览、真实释放提交；完整六位HEX自动提交，取消只丢未提交编辑。完整实现职责见[实现设计](../设计/实体显名与样式配置实现设计.md)。

既有非图形联合回归、独立Host/存储/安装恢复、完整实现审查与后续窄复核按各自实际执行时点复用；c1候选的默认微调有Debug/架构及两次顺序clean Release/架构、30产物和15包文件/ZIP一致证据，不称完整联合套件在c1重跑。详细来源、实际包/回执与合并结果保留在[#55](https://github.com/Kuroneko2020/JueMingR/issues/55)。本次收口仅文档检查及独立窄复核，不重建、重装或重测，继续保留c1安装与恢复材料；主线合并SHA不改写安装来源。

体验接受只覆盖所有者实际使用场景。交付环境在GraphicsDevice创建前XNA adapters=0，相关图形检查和代表原资源预览未执行/未查看的记录保留，不能生成像素、布局或图形状态PASS。全部Windows/IME、字体材质、分辨率、特殊多段对象、游戏内三种拓扑、极端规模、异常断电及FPS未获逐项全覆盖证据；后续相关变更或稳定基线验收按风险补证，正式发行前继续处理剩余责任。Legacy V0、静态/fixture、自动验证、指定实机与体验接受分别记录；#48延期风险不由本批接受关闭或豁免。

## 9. 后续验收候选

这些是提取阶段整理的验证入口，**该提取阶段全部未执行**；后续实现证据与本次接受边界见[第8.5节](#85-实施与接受的边界)。不把下表倒填为逐项通过记录，也不把它变成让用户检查JSON的任务。
