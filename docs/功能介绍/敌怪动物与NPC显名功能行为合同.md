# 敌怪、动物与 NPC 显名：行为提取与需求候选

**性质：Legacy 静态行为提取与新版需求候选。三个新版显名功能未实现，具体范围待所有者确认；本文不是已接受或已验证的实现合同。** 对应[任务 #53](https://github.com/Kuroneko2020/JueMingR/issues/53)，当前阶段只查[总跟踪 #7](https://github.com/Kuroneko2020/JueMingR/issues/7)。

本批按所有者明确决定成组提取、后续按技术共性共同设计；本轮没有授权编码或建设公共框架。Legacy 源码固定为 `6ac6356c0c564f43284590c168855e96c50e13f7`，R 基础固定为 `bf56e93627e5476e4f1a8f3a1628f107d4e1ea5c`。未跟踪说明另记文件身份。本文证据为 **V0 静态提取**：C 表示实际代码，B 表示测试源码中的断言，D 表示旧说明/历史记录，A 表示所有者明确决定；“建议”和“未查明”不混入旧事实。没有运行旧程序、构建、行为测试、游戏或性能采样，测试源码不记为本轮通过。
...
**推荐首轮完整交付三项显示、敌怪两行生命值、NPC 两种模式、各自样式配置与默认恢复、可靠保存、现有 F5 真实入口和公共快捷键。** 不缩成只有三个名字的演示。建议同时避免旧路径里的箱子记录副作用、模式恢复分叉、过期身份和远处对象挤掉屏内名额；这些是明确的新建议，不冒称旧版已经修好。

当前 R 可复用 F5 行控件、字体度量、输入仲裁、偏好保存、统一键鼠快捷键及 Session 生命周期。三条显名行目前只是外形；没有真实后端或动作注册。实体观察和世界标签绘制仍需最小补齐，不建设全游戏实体平台。产品问题集中在[第 8 节](#8-新版候选与待决定问题)，技术实现选择不交给所有者猜。
...
**新版建议**：按钮和快捷键走同一正常模式命令；恢复最后明确选择的名字/类型，尚无历史时提供可用的首次模式。Q2推荐首次“名字”。这保留两个模式，不将其强制压成bool，也不复制旧独立热键状态。[R04]
...
候选均待确认。沿用[Legacy平替规则](../规范/Legacy平替与行为契约规则.md)保留有意义用户结果，沿用[架构](../设计/总体架构与状态所有权.md)限制职责。不是本轮编码授权。

| 候选 | 推荐首轮结果 | 差异与边界 |
| --- | --- | --- |
| 三个真实功能一起交付 | 敌怪名称+当前/最大生命两行；动物名称；NPC名字/类型两种模式；真实F5控制、独立样式及公共快捷键 | 不延期血量、模式、配置和必要边界，不用公共空架子代替交付 |
| 保留分类与生命语义 | 按已提取分支建立有限目标/反例；多段不重复错误相加，目标消失/变形不显示旧身份 | 原版 .8 的实际字段和分段身份仍需后续目标版本证据；不宣称旧 .6/.7 表涵盖 .8 全部对象 |
| 独立样式、正常操作可恢复 | 三项各自颜色/字号；恢复默认只影响本项；配置操作与世界显示值一致 | 具体控件沿用R风格；是否保留即时生效流程由Q项确认，不移植旧UI内部状态 |
| 保存与输入 | 偏好按现有R方式在内存立即生效、合并保存；失败清楚说明仅本次有效并保护原件；快捷键仍在可靠保存后替换有效绑定 | 偏好和快捷键的生效时点不同，不能统一写成“点下即已保存”；不复制SaveAll或单项热键服务 |
| 只做本批显示 | 无标签命中、点击、NPC交互、捕捉、攻击、修改原版名字/生命或额外实体网络协议 | 剥离旧共享Draw里的箱子记录/旧键导入副作用；不迁移相邻箱子功能 |
| 合理可见范围与更新 | 保留有依据的距离/屏幕关系，及时处理身份、内容、字体/语言/样式变化；关闭后停止本批领域工作 | 建议避免固定索引前120挤占屏内对象；不增加自动避让、全图同步或测量平台 |

静态高亮、宝箱/牌子/墓碑显名、信息窗总体拖动、鱼获预测、地图及自动交互不纳入。旧配置导入独立于功能平替，不读真实文件、不建迁移中心。

### 五个待决定问题

| 问题 | 推荐 | 理由/用户可见取舍 |
| --- | --- | --- |
| Q1 首轮是否整体采用本节完整范围与必要纠偏？ | 是：三项、两行血量、两种NPC模式、样式、保存、F5/快捷键一起交付；保留本地玩家死亡但仍active时对合格可读对象的显示目标，修正串名/错血/远处名额挤占风险，移除附带箱子记录 | 避免缩水或空架子；按客户端数据表达，不增加自动交互、全图同步或避让。具体.8能力要验证，不能靠未知填值 |
| Q2 NPC从关闭再次开启怎样选模式？ | 按最后一次明确选择的名字/类型恢复，按钮与快捷键一致；从未选择时用“名字” | 解决旧UI与热键记忆分叉；不用先通过某种关闭路径才能第一次开启 |
| Q3 首次默认与样式是否保留当前源码值？ | 三项默认关闭；三色及0.70字号保留，血量按−0.13；金色动物继续固定金色 | 保留已有辨识与独立配置，不把旧说明0.80样本当默认；若希望金色动物也跟随自选颜色，应明确改变该例外 |
| Q4 配置操作是否保留旧主要流程并统一退出？ | 保留HSL预览/松手生效、六位HTML提交、字号按钮和本项默认恢复；新版切页、关闭、失焦结束未提交编辑，已提交选择不回滚 | 保留完整调节能力；有意消除旧切页隐藏后留草稿的差异。保存失败清楚提示仅本次有效，不增加一层假确认 |
| Q5 首轮是否导入旧显名配置/绑定？ | 首轮不导入，使用R默认或重新设置 | 只有少量可重建偏好；公共快捷键沿用现有R文件，避免误带旧独立模式记忆。代价是首次需重设旧自选样式/绑定，功能主体仍完整 |

这些问题只请所有者决定体验/范围。读取字段、坐标变换、扫描时序、锁、缓存键及最终类型组织由后续实施者依证据设计，不逐项转交所有者审批。
...
本任务没有运行构建、测试、游戏、宿主或旧二进制，没有新增测量/诊断能力、公共框架、接口桩、注册占位或实现。需求确认后另开同批设计/实施任务，不能把本文或文档检查通过视作三个功能已可用。
