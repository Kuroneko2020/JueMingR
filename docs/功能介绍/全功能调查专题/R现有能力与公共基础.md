...
`NativeNpcObservation`是实际共享NPC事实读取器。每次完成游戏更新换epoch，对槽按Basic/Direction/Danger/Housing demand补缺字段。方向不强迫所有消费者准备名字、血量排版；实际消费者由Composition Root共用同一实例。它按 demand 组缓存，不是严格每字段只读一次：Direction/Danger 都读 life，Housing 复用中心还取决于 Direction 是否已经读取（R06源码51、59、67行）。显名/方向等消费者仍各自遍历、筛选、排序或格式化，不能因共享读取器就声称整个组合只扫一次。目标槽的原版对象身份、generation与业务选择策略仍需领域复核。（R01、R06）
...
| R10 | [ItemSelection.cs 24–74](https://github.com/Kuroneko2020/JueMingR/blob/df8a029873176276d0f620d32b0ac02ae9ea7744/src/JueMingR.TerrariaHost/Items/ItemSelection.cs#L24-L70)：Open/Select/Confirm，主包类型选择边界 |
...
本篇已有入口支撑总报告的 B01–B06 候选，但接受基础不等于所有未来消费者都适用或现物永远无需调整。新消费者必须走现有会话、输入、保护与持久化出口，再依据真实规模、阶段和语义提出窄扩展。

重点保护显名/方向的独立遍历及需求释放、物品名单的打开时背包快照、死亡图标专用锚点和各文档不同保存语义。对首次接入产生的新证据维护现行合同，不把本篇固定生产SHA改写为后来的实现状态。
