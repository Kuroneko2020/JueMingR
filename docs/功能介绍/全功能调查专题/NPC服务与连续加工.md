...
**产品待决定**：护士何时触发、是否保留现有对话、费用上限、付费反馈形式、与药水的优先级和费用未知时的暂停方式。**正确性责任**：付款及治疗结果有据可查，不重复扣费，已请求与最终完成必须区分；不能把是否报真实结果交给产品选择。原版或服务器未明分支另列证据缺口，尚未构成六项 R 产品契约。
...
**产品待决定**：是否每有一点就领、阈值、领取/到账的反馈形式及与自动存钱的配合。**正确性责任**：领取、世界金币生成、实际到账不能混成同一成功；多人未确认不得报完成或在幂等性未证实时重试。当前只确认 Legacy 行为，不将旧行为提升为 R 决定。
...
**产品待决定**：重铸是否纳入及其预算、前缀选择呈现、预先命中时是否允许手动重铸、命中后何种输入重新开始、无效名单的反馈。**正确性责任**：提供正常付费重铸时须覆盖原版外层收费和前缀变化的真实关系，不因只调用内部方法漏费，也不重复收费；部分执行和未确认不得伪装未操作或完成。具体词缀身份与费用核对由实施者负责。
...
V8 中通过 Z 上述选择门的完整输入表如下，共 **16 种、7 个模式**。它绑定 .8 静态表与冻结 Z 选择器，不是任何版本的永久数字预算，也不包含下面原版额外交易的输入：
...
因此明确分三层：**Z 实际自动选择与生产执行**为 §6.1 的16输入/7模式，经 §6.3 的 ItemUse 链；**原版额外支持**为本节52条交易，全文保留作资料；**R 可选扩展**是否自动化这些交易须在 G08 另定。§6.3 已记录排队间换物可让执行偏离原候选，这种错误路径不能充当 Z 支持额外自动交易的证据，也不宣称旧链在任何异常交错下都绝不触达原版交易分支。
...
**产品待决定**：默认允许消耗哪些材料、普通机/叶绿机选择、保留数量/收藏、是否自动换材料/临时借用武器槽、是否扩展52条交易及未知时的提示方式。**正确性责任**：执行时仍为预期物品与机器、不覆盖玩家新选槽、消费和产物逐层确认、多人未知不报成功并停止不安全重试。如何证明这些责任是实施与补证工作，不是是否接受错误的选择；均未授权实现。
...
4. 重铸预算、词缀选择呈现、预先已命中、错误名单与按住会话重新开始规则；正常收费链必须完整且不重复扣费是实施责任。
5. 连续开袋是否始终锚定当前堆叠/类型，是否跳过收藏、保留袋/钥匙，产物满包与多人待定时是否继续。
6. 提炼是否可自动耗掉全背包材料、是否优先叶绿机、是否归还原武器槽；52 条交易是否另外定义模式。
7. **实施责任**：每个不可逆操作以真实消费和结果分层确认；异常、关闭、死亡、换角色/世界时如实区分未开始、已发生、部分和未知，不承诺撤销已发生效果。
8. **技术接入建议**：先复用 R 已有输入、Session、NPC/Tile 观察、来源与物品保护；尚未存在的 NPC 服务/加工操作保持候选，不以旧类/队列名作为实施合同。具体原版未明副作用归证据缺口，只暂停依赖部分。
...
本篇消费 B01/B02/B03/B04：加工名称/图标/有证据的输入输出关系可复用目录，机器和NPC当前资格、付费、实际消费、产物归属仍由各服务拥有。三层提炼范围见§6.1/§6.4；52条原版交易资料不自动变成Z恢复或R扩展合同。

C02组合要求加工与存卖扔/存钱共享可靠获得事实和保护入口，不能各写一套后处理。开袋生成、提炼落地、税款生成与实际入袋有不同因果；未进包不给非币新拾取资格，钱币不套非币资格。稳定材料/准入不重做全部计划，消费/机器变化与真实获得仍及时更新，结果未知时不靠重试补“成功”。
