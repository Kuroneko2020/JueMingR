...
**新版行为合同候选，尚未获所有者接受，四项尚未在 R 实现。** 本文交付 Legacy 静态行为提取、当前原版必要查证及 R 首轮建议；不是实施授权。任务与动态状态查 [调查 Issue #79](https://github.com/Kuroneko2020/JueMingR/issues/79)，上位边界见 [Legacy 平替规则](../规范/Legacy平替与行为契约规则.md)、[总体架构](../设计/总体架构与状态所有权.md)和[配置设计](../设计/配置持久化与用户数据布局.md)。
...
- **所有者已决定的范围**：调查这四项、记录与展示分开、不静默删历史、未知不假零、不放宽游戏操作安全门；允许候选文档交付。**尚未决定**：本文件提出的新版具体行为与取舍。旧文档声称、代码实际行为、测试意图、静态已证问题、推断和新版建议分别标明。
- 验证层级为 **V0 静态提取**。未运行产品测试、Legacy、探针、游戏、多人、图形或性能测量。静态路径可证明发生某种输入时的代码后果，不能证明用户已经遇到事故、实际卡顿、毫秒收益或 FPS 改善。
...
推荐按一个**角色×世界记录职责**组织死亡事实及计时状态，死亡次数、详情、地图只是三个死亡记录消费者；计时另有自己的观察算法和累计状态。它们可以共享身份与机械文件机制，不机械建四套 Feature/服务/记录，也不合并为一个承担 UI、地图投影和全部存储的巨型类。
...
当前原版 `.8` 的 `Player.KillMe` 在 creative god mode 或已经 `dead` 时直接返回；有效路径增加原版 PVP/PVE 计数、设置最近死亡点及时间、设 `dead=true`，生成原版死亡文本，再广播并供墓碑消费。因此“调用了 KillMe”与“发生新死亡”必须分开，持续倒计时不是新死亡。死亡取消、重复回调、刚进世界已死、复活再死及会话切换的具体旧去重边界见本节补充证据；R 候选必须以一次真实完成的死亡对应一个事件身份。（N02）
...
因此 R 候选优先捕获**原版当次已经生成的结果与当时可确认的原因值**，不得重新触发随机生成、广播、墓碑或解析聊天重建全服历史。确切可复用的只读捕获位置仍需后续窄设计核验；若安全取得不到原句，保留当次可信来源事实或明确原因缺失，不能延后从已复用槽补造名字。首轮不要求给历史记录重新翻译，保留当次语言文本；显示界面自身文案仍随当前资源更新。
...
R 首轮建议保留两列、升序、6 条和最早页作为等价起点，补充当前行原因的只读完整换行查看，限定在视口内；历史与页面游标分开，稳定内容不重排。真实会话切换强制关旧详情、取消旧请求，重新打开读新 pair；F5 暂藏/切页保留本会话页码。新死亡更新计数并仅在影响当前页时准备页面，不用每帧排序全部历史。完整原因查看的具体视觉由后续 UI 验证确认，不新增搜索、删除、导出或统计。（L02、L03、L09）
...
**推荐候选**：累计本角色在此世界被实际观察到的游戏时间推进，计入睡觉、日/月晷和旅行倍率；冻结/实际暂停不增加，直接设时、回拨与网络校正不算游玩增量。不补断线、离线或未加载历史；普通客机只能表述本客户端的观察累计，不保证服务器完整历史。
...
更早的记录仍在文件，可在详情查看。相同坐标和同 EventId 不去重；旧到新绘制时后面的命中覆盖前面的 hover 文字。图层里的 `drawn>=256` 是第二道上限，不能抵消前面全史读取成本。**存储保留、候选筛选与最终绘制数量是三个不同合同。** R 不自动继承 256 为固定预算，更不能以它截断存储历史。
...
推荐共存候选：不修改原版字段或寿命；能可靠对应**同一次死亡**时让原版承担当前最近点，R 暂不重复画该事件，原版不再呈现时由历史层承担。不能仅因坐标接近就吞掉另一次死亡，不能靠修改原版标志实现去重；准确对应的事件身份仍须后续 Host 验证。无法可靠对应时保留两者并说明限制。
...
死亡层只写 `ref text`，没有点击动作、鼠标捕获或消费拖动/缩放/ping；原版拖图与 ping 发生在它之前。这仅证明该层没有新增点击行为，不能代替整个地图输入实测。R 候选只读 hover 应限定视口并复用现有字体度量，保留原版拖动、缩放、ping；F5 进入全屏地图时沿既有壳层退出，不另争输入。
...
上述来自 R03、N06、N09；没有读取玩家真实档案。推荐首轮沿既有“明确文件身份×世界 GUID”口径，拒绝弱显示名 fallback，明确移动/云迁移/同 GUID 副本限制；不为了本批另造角色 GUID、改存档或新增服务器协议。迟到 pair 只可接收同一 Runtime generation 暂存事实，无法证明归属就保留未知，不写入共用桶。
...
推荐一个领域记录 owner 管理当前 pair 的已确认死亡事件、历史读取状态和未保存新增；计时累计具有独立子状态。次数从同一已知事件修订派生，详情和地图读取相应投影。摘要如保留，只作可校验派生加速信息，不另作可永久落后的事实。
...
| **一个记录事实 owner，窄死亡/时间观察，独立 UI 与地图投影** | 推荐；复用现有 Runtime、身份来源、机械文件/worker及 UI，但需补真实死亡文本捕获、实际计时推进、历史页/候选和地图适配。只构造本组必要能力 |
...
| 能力 | 已有 R 基础 | 本组缺口/限制 |
| --- | --- | --- |
| Runtime/Session | `SingleFeatureRuntime.AddFeature/Update`、`ItemSessionProbe`、既有唯一 Host handoff | 复用代次与进入/退出；不新增第二套会话。接入 profile 仍需后续授权 |
...
以下是按[玩家可见文案](../设计/F5控制界面样板与UI基础.md#玩家可见文案)阅读完整拼接后的候选，仅供对应状态使用；不在 Draw 扫描/替换工程消息，不重审上一批全部文案。
...
| 正常天数 | “世界天数　12 天” |
| 死亡点常驻简介 | “在全屏地图显示这个角色在当前世界记录的死亡点。” |
...
| 原因缺失 | “死亡原因未记录” |
| 地图有限展示 | 在相关说明中标明“地图仅显示最近的一部分记录，完整记录可在详情查看。”；数量变化不每帧通知 |

正常读取、记录及成功保存不刷聊天。错误只在原因变化时提醒，已有内存信息继续可用；没有支持的操作不附“请重试/删文件”。历史文本保留来源含义，完整原因查看与地图长文本必须核验实际视口呈现。

## 8. 静态工作量与成本目标

`N` 为同 pair 的全部历史行数，`P=6` 为旧详情一页，`K≤256` 为旧地图候选。表中频率均来自调用链，不能换算为固定毫秒或 FPS。Legacy death recorder、history cache、marker cache、playtime service 各有自己的 `SyncRoot`，不存在可据此称赞的共享后台 worker。（L03–L16、L20）

| 状态/入口与阶段 | Legacy 实际频率、规模与成本 | R 推荐必要工作、owner与失效 |
| --- | --- | --- |
...
本节只列建议，**本轮不编写、不运行这些测试，也不先建探针**。Legacy 已有死亡格式/关闭仍记录/身份未知、摘要优先/分页/坏行、时间跨昼夜/大跳变、地图候选限制等测试源码，分别证明其断言意图；不覆盖下表全部真实边界，更不等于旧测试当前通过。（L19）
...
以下四项共同构成推荐首轮方案；其余内部命名、类拆分、工具与具体实现由后续授权任务负责。本候选不自动推进实施。

| 待决定结果 | 推荐与理由 | 不采纳的后果/替代代价 |
| --- | --- | --- |
| “世界天数”如何对待加速与调时 | 保留从0起的观察累计，计睡觉/日月晷/旅行倍率，排除直接设时与网络校正；与旧用途一致并纠正误计 | 继续差值阈值会有校正误计/跨圈漏计；改现实在线时长则改变功能含义 |
| 角色历史如何跨文件/云/世界副本识别 | 首轮沿可靠角色文件身份×世界GUID，不猜同名、不自动合并；普通原版改名保持路径时延续 | 希望转云/搬文件或同GUID服务器副本自动延续/分开，需要另定可靠关联；否则会出现分档或同GUID合档，不能悄悄承诺连续 |
| 死亡详情的阅读方式 | 保留旧两列、升序每页6条、打开最早页；补完整原因只读查看，换会话关闭旧页 | 完全保留旧省略会看不到长原因；改最新优先/更多列需明确改变旧习惯，首轮不自动增加 |
| 地图的历史范围与原版共存 | 默认关/仅全屏，有限最近候选且完整历史留详情，具体预算由后续证据定；可靠对应同一次死亡时避免重复图 | 要求无限同时画会增加大历史成本；完全保留双图会有错位/hover覆盖；用接近坐标猜去重会漏显示真实不同死亡 |

原因捕获、安全保存、未知不假零、关闭仍记录属于推荐方案必要正确性边界，不交给玩家选择是否牺牲正确性换成本。首轮不含旧数据导入、搜索/删除/导出/统计、传送/寻路/地图编辑、服务器部署或协议、通用记录平台。无法安全取得当次原文/时间贡献时应具体报告缺口，不以更改游戏行为绕过。
