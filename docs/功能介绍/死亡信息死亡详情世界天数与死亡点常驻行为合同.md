...
**首轮行为目标已获所有者接受，四项实现与验证另行交付。** 所有者已接受本文的时间累计、身份和详情口径，并明确地图数量四档及同一弹窗的全文阅读方式，见[接受与实施授权](https://github.com/Kuroneko2020/JueMingR/issues/79#issuecomment-5678391471)。本文保留 Legacy 静态提取与当前原版证据，需求收口不表示四项已经实现或通过实机验证；生产实施进入独立任务，不追加到调查 PR。任务与动态状态查 [调查 Issue #79](https://github.com/Kuroneko2020/JueMingR/issues/79)，上位边界见 [Legacy 平替规则](../规范/Legacy平替与行为契约规则.md)、[总体架构](../设计/总体架构与状态所有权.md)和[配置设计](../设计/配置持久化与用户数据布局.md)。

当前开发实现、构建/安装与待体验状态查 [实施 #81](https://github.com/Kuroneko2020/JueMingR/issues/81) / [Draft PR #82](https://github.com/Kuroneko2020/JueMingR/pull/82)。以下调查段落保持原V0身份；当前实现说明见第12节，不把后续检查倒写为Legacy运行证据。
...
实施必须交付四项真实功能、对应正确性/自动工作量回归、公共入口接线、有限负对照、必要真实资源UI核对、独立只读增量审查、固定干净提交的完整开发包及授权安全安装；实现 PR 保持 Draft，等待所有者实测接受。需求文档自身不证明这些结果。首轮不含旧数据导入、搜索/删除/导出/统计、传送/寻路/地图编辑、服务器部署或协议、通用记录平台。无法安全取得当次原文/时间贡献时具体报告缺口，不以更改游戏行为或永久缺失掩盖未完成。
...
F5地图页新增固定“死亡信息”“世界天数”及“死亡点常驻”行。点击“详情”按现实时间升序阅读，每页最多六条，显式打开从最早页起；时间保留死亡当时的时区偏移，相同UTC用稳定事件身份并列。点击原因进入同窗全文，按宽度换行、滚轮阅读，返回保留页码；显式关闭重开回最早页。小视口只滚当前内容，逻辑分页不变。
...
真实实现、文件职责、有限容量和检查入口见[实现设计](../设计/死亡记录世界天数与地图死亡点实现设计.md)。已具备领域/真实隔离存储、固定原版IL受控执行、完整Host接纳与F5输入、生产地图CPU循环检查；具体候选与正式执行结果以#81为准。原资源预览在设备创建前因XNA无显示适配器而未执行，视觉/GPU/完整原版地图共存、实际游玩、多人与FPS不在已验证范围内。实施任务保持OPEN、PR保持Draft，等待所有者体验接受。
