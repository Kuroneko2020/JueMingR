...
自动出售、丢弃与堆叠及该候选已有的共享基础局部修正已获所有者接受，随 [PR #47](https://github.com/Kuroneko2020/JueMingR/pull/47) 合并进入 `main`。三项由可靠取得触发，使用 F5 内联名单与明确开关，丢弃头顶提示可单独保存，三项快捷键绑定仍已撤除。使用与接受边界见[物品自动处理合同](docs/功能介绍/自动存放功能行为合同.md#r-使用与接受边界)，固定候选、实际安装、验证与反馈见 [Issue #46](https://github.com/Kuroneko2020/JueMingR/issues/46)。特定旧槽位报文跨批次疑点由所有者限定接受并[延期调查 #48](https://github.com/Kuroneko2020/JueMingR/issues/48)，不等于风险已排除或全部多人、异常网络、性能场景通过；本次接受不是正式发行，其余 Legacy 功能仍是后续范围。
