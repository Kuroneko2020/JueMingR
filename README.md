...
[配置持久化合同](docs/设计/配置持久化与用户数据布局.md)规定：在已验证 `Terraria.exe` 同级的 `JueMingRData/config/` 下，用 `ui.json` 保存 F5 主窗口位置，用 `features/biome-display.json` 保存真实群系显示的用户期望开关。同一游戏安装内跨角色和世界共享偏好；没有 AppData/Documents fallback，不自动导入 Legacy 数据。该功能的分支实施、主线生效和实机验收按设计及任务 Issue 分别确认，不能沿用旧 UI 验收。
