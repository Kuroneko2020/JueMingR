...
{
    internal class TextEditInput
    {
        private readonly object owner = new object();
        private readonly ITextEditSession workspace;
        ...
        private TextEditBuffer bound;
        private TextEditBuffer prepared;
        ...
        internal TextEditInput(ITextEditSession workspace, INotesClipboard clipboard, INotesIme ime = null)
        ...
        internal void AfterSample(bool active, NotesTextLayout layout, KeyboardState physicalSample, bool focused)
        {
            sample = physicalSample;
            if (!focused) { Release(false); return; }
        ...
        private void ProcessText(bool suppressEnter, bool suppressEscape)
        {
            ...
            {
                if (bound.SingleLine) { FinishComposition(false); if (!HasComposition) workspace.RequestFinish(); }
                else bound.Insert("\n");
            }
        }
        private void ProcessKeys(NotesTextLayout layout)
        {
            TextEditBuffer editor = workspace.Editor; if (editor == null) return;
        ...
        {
            committed.Clear();
            for (int i = 0; i < Math.Min(Main.keyCount, Math.Min(Main.keyInt.Length, Main.keyString.Length)); i++)
