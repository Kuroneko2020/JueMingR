...
namespace JueMingR.TerrariaHost.F5
{
    ...
    internal sealed class F5Shell
    {
        ...
        { this.biome = biome; this.preferences = preferences; this.hostItems = hostItems; notes = new NotesPresentation(hostNotes.Workspace); notes.Attach(State);
            ...
            if (hotkeys != null || labels != null || worldTargets != null || worldObjects != null || information != null || deaths != null || maps != null) this.inputState.ClaimsHotkeyPointer = ClaimsPopupPointer;
            ...
            State.BeforeLeave = page => { if (MapPopup != null && !MapPopup.TryLeave(() => { if (page < 0) State.Close(); else State.Navigate(page); })) return false; if (prior != null && !prior(page)) return false; HotkeyPopup?.Close(); StylePopup?.Close(); DeathPopup?.Close(); return true; };
        }
        ...
        { HotkeyPopup?.Click(id, rect, State.Layout.Generation, State.Page, clickClock.ElapsedMilliseconds); if (HotkeyPopup != null && HotkeyPopup.Visible) { StylePopup?.Close(); DeathPopup?.Close(); } }
        ...
        internal void RequestInformationAdjustment()
        {
            if (!CanAdjustInformation || information.Adjustment.Active || adjustmentPending ||
                HotkeyPopup != null && HotkeyPopup.Capturing || StylePopup != null && StylePopup.HasCapture || DeathPopup != null && DeathPopup.Visible || MapPopup != null && MapPopup.Visible || items != null && items.Selecting) return;
        ...
        internal bool CanExecuteMerchantInput
        {
            get { return !failed && State.Ready && State.Visible && CanPresentNow && inputState.CanStartActions &&
                ...
                !(HotkeyPopup != null && HotkeyPopup.Visible) && !(StylePopup != null && StylePopup.Visible) && !(DeathPopup != null && DeathPopup.Visible) && !(MapPopup != null && MapPopup.Visible) &&
        ...
        internal void ProcessInput()
        {
            try
            {
                ...
                if (StylePopup != null && StylePopup.Visible || DeathPopup != null && DeathPopup.Visible || MapPopup != null && MapPopup.Visible) renderer.RefreshResources();
                ...
                bool popupPointer = HotkeyPopup != null && HotkeyPopup.BlockPointer || StylePopup != null && StylePopup.BlockPointer || DeathPopup != null && DeathPopup.BlockPointer || MapPopup != null && MapPopup.BlockPointer || inputState.HotkeyPointerOwned;
                State.Update(new F5Input
                {
                    ...
                    PageWheelHandled = HotkeyPopup != null && HotkeyPopup.ConsumeWheel || StylePopup != null && StylePopup.ConsumeWheel || DeathPopup != null && DeathPopup.Visible || MapPopup != null && MapPopup.Visible || inputActive && notes.Wheel(pointer.X, pointer.Y, PlayerInput.ScrollWheelDeltaForUI)
                });
                if (!State.Visible) { HotkeyPopup?.Close(); StylePopup?.Close(); DeathPopup?.Close(); MapPopup?.Suspend(); }
                ...
                { HotkeyPopup?.Close(); StylePopup?.Close(); DeathPopup?.Close(); MapPopup.Open(State.Command == F5Command.ExplorationDetails); }
                ...
                { HotkeyPopup?.Close(); StylePopup?.Close(); DeathPopup.Open(State.Command == F5Command.DeathConfigure, State.Page); }
                ...
                else if (State.ClickedControl != null && GuidanceControls.IsStyle(State.Command) && renderer.GuidanceControls != null && renderer.GuidanceControls.Available(State.Command))
                {
                    ...
                    renderer.DeathControls?.Execute(State.Command); renderer.MapControls?.Execute(State.Command);
                ...
                bool gameplay = inputActive && !(information != null && information.Adjustment.Active) && !adjustmentPending && !Main.blockInput && !Main.drawingPlayerChat && !Main.editSign && !Main.editChest &&
                    ...
                    !(HotkeyPopup != null && HotkeyPopup.Visible) && !(StylePopup != null && StylePopup.Visible) && !(DeathPopup != null && DeathPopup.Visible) && !(MapPopup != null && MapPopup.Visible) && !(items != null && items.Selecting) &&
        ...
        private void ConsumeSample()
        {
            ...
            if (State.ConsumeWheel || notes.ConsumeWheel || items != null && items.ConsumeWheel || HotkeyPopup != null && HotkeyPopup.ConsumeWheel || StylePopup != null && StylePopup.ConsumeWheel || DeathPopup != null && DeathPopup.ConsumeWheel || MapPopup != null && MapPopup.ConsumeWheel)
        ...
        internal void AfterUpdate()
        {
            try
            {
                ...
                RestorePositionWhenLoaded();
                CheckLabelSession();
                DeathPopup?.CheckSession(); MapPopup?.CheckSession();
                if (deaths != null) State.Layout.SetDeathInformation(deaths.CountText, deaths.DaysText);
                if (worldObjects != null) State.Layout.SetWorldObjectSettings(worldObjects.Preferences.Value);
                ...
                if (!Main.gameMenu && !Main.hideUI)
                {
                    ...
                    deaths?.TakeFeedback(displayPreferenceFeedback); maps?.TakeFeedback(displayPreferenceFeedback);
            ...
            try
            {
                ...
                else if (!failed)
                {
                    ...
                    if (State.Visible && State.Ready)
                    {
                        ...
                        bool hintsBlocked = !inputState.CanPrepareText || HotkeyPopup != null && HotkeyPopup.Visible || StylePopup != null && StylePopup.Visible || DeathPopup != null && DeathPopup.Visible || MapPopup != null && MapPopup.Visible;
        ...
        internal void CloseAndSubmitPosition() { CancelInformationAdjustment(false); HotkeyPopup?.Close(); StylePopup?.Close(); DeathPopup?.Close(); MapPopup?.Suspend(); State.Close(); notes.Suspend(); items?.Suspend(); SubmitPosition(); }

        internal void CancelForFocusLoss()
        { CancelInformationAdjustment(false); HotkeyPopup?.Close(); StylePopup?.Close(); DeathPopup?.Close(); MapPopup?.Suspend(); State.CancelForFocusLoss(); notes.Suspend(true); items?.Suspend(); RestoreLeases(); }
