    ...
    internal static class Phase0SHarmonyWorker
    {
            ...
            Assembly harmonyAssembly)
        {
            ...
            try
            {
                ...
                postfixContext.InstallInformationSources();
            ...
            {
                postfixContext?.DisposeInformationSources();
        ...
        private static void EnsureEntityLayer(List<GameInterfaceLayer> layers, bool setupComplete)
        {
            ...
            if (context == null || !(context.PackageId.StartsWith("entity-labels-", StringComparison.Ordinal) || context.PackageId.StartsWith("world-targets-", StringComparison.Ordinal) || context.PackageId.StartsWith("world-object-text-", StringComparison.Ordinal) || context.PackageId.StartsWith("information-summary-", StringComparison.Ordinal) || context.PackageId.StartsWith("direction-equipment-", StringComparison.Ordinal) || context.PackageId.StartsWith("death-history-", StringComparison.Ordinal) || context.PackageId.StartsWith("map-markers-exploration-", StringComparison.Ordinal) || (context.PackageId.StartsWith("footprints-", StringComparison.Ordinal) || (context.PackageId.StartsWith("item-browser-", StringComparison.Ordinal) || (context.PackageId.StartsWith("favorite-quick-items-", StringComparison.Ordinal) || (context.PackageId.StartsWith("coin-deposit-", StringComparison.Ordinal) || (context.PackageId.StartsWith("about-help-feedback-", StringComparison.Ordinal) || (context.PackageId.StartsWith("recovery-buffs-services-", StringComparison.Ordinal) || context.PackageId.StartsWith("continuous-processing-", StringComparison.Ordinal))))))))) return;
            try
            {
                // Capture's early return precedes this Game-scale layer. Normal
                // menus and F5 follow it; neither UI scale nor drawToScreen is a
        ...
        private static void InsertF5Layer(List<GameInterfaceLayer> layers, string name, string anchor, GameInterfaceDrawMethod draw, InterfaceScaleType scale = InterfaceScaleType.UI)
        {
            ...
            if (own == 0) layers.Insert(anchorIndex, new LegacyGameInterfaceLayer(name, draw, scale));
        }
        ...
        {
            // One shared consumer replaces the former fixed-position biome
        ...
        private sealed class PostfixContext
        {
            ...
            internal Map.HostMapFeatures MapFeatures { get; private set; }
            internal Footprints.HostFootprints Footprints { get; private set; }
            ...
            {
                if (!(PackageId.StartsWith("information-summary-", StringComparison.Ordinal) || PackageId.StartsWith("direction-equipment-", StringComparison.Ordinal) || PackageId.StartsWith("death-history-", StringComparison.Ordinal) || PackageId.StartsWith("map-markers-exploration-", StringComparison.Ordinal) || (PackageId.StartsWith("footprints-", StringComparison.Ordinal) || PackageId.StartsWith("item-browser-", StringComparison.Ordinal) || (PackageId.StartsWith("favorite-quick-items-", StringComparison.Ordinal) || (PackageId.StartsWith("coin-deposit-", StringComparison.Ordinal) || (PackageId.StartsWith("about-help-feedback-", StringComparison.Ordinal) || (PackageId.StartsWith("recovery-buffs-services-", StringComparison.Ordinal) || PackageId.StartsWith("continuous-processing-", StringComparison.Ordinal))))))) || informationHooks != null) return;
            ...
            internal void InitializeRuntime(bool enabled)
            {
                ...
                bool quickPackage = (PackageId.StartsWith("favorite-quick-items-", StringComparison.Ordinal) || (PackageId.StartsWith("coin-deposit-", StringComparison.Ordinal) || (PackageId.StartsWith("about-help-feedback-", StringComparison.Ordinal) || (PackageId.StartsWith("recovery-buffs-services-", StringComparison.Ordinal) || PackageId.StartsWith("continuous-processing-", StringComparison.Ordinal)))));
                ...
                if (itemPackage)
                {
                    ...
                    if (quickPackage)
                    {
                        ...
                        KeepFavorited = new KeepFavorited.HostKeepFavorited(QuickItems);
                        runtime.SharedRuntime.AddFeature(KeepFavorited);
                    }
                    if(PackageId.StartsWith("continuous-processing-",StringComparison.Ordinal))
                    {Processing=new Processing.HostProcessing(gameDirectory,runtime.SharedRuntime,items,Input);runtime.SharedRuntime.AddFeature(Processing);}
                    runtime.SharedRuntime.AddFeature(items);
                    if (QuickItems != null) runtime.SharedRuntime.AddFeature(QuickItems);
                    if ((PackageId.StartsWith("coin-deposit-", StringComparison.Ordinal) || (PackageId.StartsWith("about-help-feedback-", StringComparison.Ordinal) || (PackageId.StartsWith("recovery-buffs-services-", StringComparison.Ordinal) || PackageId.StartsWith("continuous-processing-", StringComparison.Ordinal)))))
                    {
                ...
                if(Processing!=null){Processing.CanInterface=()=>Shell.CanProcessingInput;Processing.BankGuardsReady=()=>Recovery!=null && Recovery.Available;}
